...
def accuracy_calculator(platform_img, gr_tr_img):
    '''
    Calculates pixel-level accuracy of predicted binary nuclear mask relative to ground truth mask

    Parameters
    platform_img: np.ndarray
        Two-dimensional array of platform binary black & white mask
    gr_tr_img: np.ndarray
        Two-dimensional array of ground truth (annotated) binary black & white mask
    '''
    
    # Ensures the inputs are binary, black & white arrays
    plat_bin_bool = np.all(np.unique(platform_img.reshape(-1)) == np.array([0, 255]))
    gr_tr_bin_bool = np.all(np.unique(gr_tr_img.reshape(-1)) == np.array([0, 255]))
    ...
    # Calculates the accuracy based on the similarity of the pixels between prediction and ground truth
...
def intersection_over_union(ground_truth, prediction):
    '''
    Calculates intersection over union (IoU) of each ground truth and predicted nucleus

    Parameters
    ground_truth: np.ndarray
        Two-dimensional array containing label masks of ground truth nuclei
    prediction: np.ndarray
        Two-dimensional array containing label masks of nuclei predicted by a platform
        
    Returns
    IOU: np.ndarray
        N X M array where N is the number of ground truth nuclei and M is the number of predicted nuclei. The array contains the
...
def F1_score_calculator(platform_img, gr_tr_img, threshold):
    ...
    aHD: float
        average Hausdorff distance of nuclear predictions relative to ground truth
    nuclei_count: int
    ...
    # Binary nuclei that shows the predicted/ground truth pairs of nuclei with IOU greater than a preset threshold
    ...
    return f1, nuclei_count, gr_tr_nuclei_count, aHD